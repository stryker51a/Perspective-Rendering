...
def equal(A, B):
    ...
    for n in A.f.keys():
        if not getitem(A, n) == getitem(B, n):
            equals = False
...
def scalar_mul(M, x):
    ...
    ans = Mat(M.D, M.f).copy()

    for n in ans.f.keys():
        setitem(ans, n, ans[n] * x)
    return ans
...
def vector_matrix_mul(v, M):
    ...
    for key, val in M.f.items():
        row, col = key
        dict[col] = dict.get(col, 0) + val * v.f.get(row, 0)
    return Vec(M.D[1], dict).copy()
...
def matrix_vector_mul(M, v):
    ...
    for key, val in M.f.items():
        row, col = key
        dict[row] = dict.get(row, 0) + val * v.f.get(col, 0)
    return Vec(M.D[0], dict).copy()
...
def matrix_matrix_mul(A, B):
    ...
    for key, val in A.f.items():
        row, col = key
        for key2, val2 in B.f.items():
            row2, col2 = key2
            if col == row2:
                dict[(row, col2)] = dict.get((row, col2), 0) + val * val2
    return Mat((A.D[0], B.D[1]), dict).copy()
