...
def mat2display(pts, colors, row_labels = ('x', 'y', 'u'),
                ...
                crosshairs=False, browser=None, output_svg=False, output_path=None):
    ...
    hpath = _create_temp('.html') if output_path is None else output_path
    with open(hpath, 'w') as h:
        if not output_svg:
            h.writelines(
                ['<!DOCTYPE html>\n',
...
def mat2png(pts, colors, output_path, row_labels=('x', 'y', 'u'), height=500):
    """ input: matrix pts and matrix colors representing an image
        result: Displays the image in a web browser
    ...
    # Draw at 2x resolution and then downsample using Lanczos resampling
    height *= 2

    rx, ry, ru = row_labels
    ymin = min(v for (k, v) in pts.f.items() if k[0] == ry and v > -10)
    xmin = min(v for (k, v) in pts.f.items() if k[0] == rx and v > -10)
    ymax = max(v for (k, v) in pts.f.items() if k[0] == ry and v < 10)
    xmax = max(v for (k, v) in pts.f.items() if k[0] == rx and v < 10)

    pixels_per_unit = height / (ymax - ymin)
    width = math.ceil(pixels_per_unit * (xmax - xmin))
    print(f"size: {width} w, {height} h")
    print(f"xmin: {xmin}, ymin: {ymin}, xmax: {xmax}, ymax: {ymax}, ppu: {pixels_per_unit}")

    im = Image.new("RGB", (width, height), color="white")
    draw = ImageDraw.Draw(im)

    pixels = sorted(colors.D[1])
    max_x, max_y = pixels[-1]
    print(max_x, max_y)

    # go through the quads, drawing each one to canvas
    for pixel in pixels:
        curr_x, curr_y = pixel

        r = _color_int(colors[('r', pixel)])
        g = _color_int(colors[('g', pixel)])
        b = _color_int(colors[('b', pixel)])

        next_x = min(curr_x+1, max_x)+1
        next_y = min(curr_y+1, max_y)+1

        # coords of corners
        x0 = (pts[(rx, pixel)] - xmin) * pixels_per_unit
        y0 = (pts[(ry, pixel)] - ymin) * pixels_per_unit
        x1 = (pts[(rx, (next_x, curr_y))] - xmin) * pixels_per_unit
        y1 = (pts[(ry, (next_x, curr_y))] - ymin) * pixels_per_unit
        x2 = (pts[(rx, (next_x, next_y))] - xmin) * pixels_per_unit
        y2 = (pts[(ry, (next_x, next_y))] - ymin) * pixels_per_unit
        x3 = (pts[(rx, (curr_x, next_y))] - xmin) * pixels_per_unit
        y3 = (pts[(ry, (curr_x, next_y))] - ymin) * pixels_per_unit

        draw.polygon([(x0, y0), (x1, y1), (x2, y2), (x3, y3)], fill=(r, g, b))

        # print(f"x: {pts[(rx, pixel)] }, y: {pts[(ry, pixel)]}, xmin: {xmin}, ppu: {pixels_per_unit}")
        # print(f"{x0}, {y0}")
